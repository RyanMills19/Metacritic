...
companies = pd.read_csv('./csv/movies_metacritic.csv')
...
home_layout = html.Div(
    [
        ...
        html.P(
            [
                html.A("Ryan", href="https://ahmedbesbes.com", target="_blank"),
                " ",
                html.A("Mills", href="https://marwandebbiche.com",
                       target="_blank"),
                " - 2020"
            ],
...
@app.callback(
    [
        Output('movie_logo', 'src'),
        Output('movie_name', 'children'),
        Output('review', 'value'),
        Output('movie_link', 'href')
    ],
    [
    ...
    [
        ...
        State('rating', 'value'),
        State('movie_name', 'children')
    ]
)
def change_title(submit_click_ts, another_title_click_ts, review_text, score, rating, title_name):
    if submit_click_ts > another_title_click_ts:
    ...
    random_movie = companies.sample(1).to_dict(orient="records")[0]

    movie_logo_url = random_movie['movie_logo']
    if not movie_logo_url.startswith('http'):
        movie_logo_url = 'https://' + movie_logo_url

    movie_name = random_movie['movie_name']
    movie_website = random_movie['movie_website']

    return movie_logo_url, movie_name, '', movie_website
...
def load_review_table(pathname):
    ...
    table = dbc.Table.from_dataframe(reviews,
                                     ...
                                     hover=True,
                                     responsive=True,
                                     header=["id", "title", "created_date", "review",
                                             "rating", "suggested_rating", "sentiment_score"],
                                     columns=["id", "title", "created_date", "review",
                                              "rating", "suggested_rating", "sentiment_score"]
                                     )
